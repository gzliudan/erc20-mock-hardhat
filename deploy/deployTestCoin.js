...
async function deployTestCoin() {
  console.log(`DO: Deploy TestCoin to ${CHAIN_NAME}`);
  const instance = await deployContract(deployer, 'TestCoin', []);
  ...
  console.log(`OK: TestCoin is deployed at ${address}, block = ${block}, hash = ${hash}`);
}

async function deployAll() {
  await deployTestCoin();
}
